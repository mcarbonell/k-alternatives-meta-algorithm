...
function loadTSPJSON(jsonFile) {
    try {
        ...
        const newError = new Error(`Error loading JSON TSP file ${jsonFile}: ${error.message}`);
    ...
    for (const file of jsonFiles) {
        ...
        try {
            ...
            if (filter) {
                if (
                    filter.name &&
                    !problem.name.toLowerCase().includes(filter.name.toLowerCase())
                ) {
                    continue;
                }
                ...
                if (
                    filter.hasOptimal !== undefined &&
                    ((filter.hasOptimal && !problem.optimalDistance) ||
                        (!filter.hasOptimal && problem.optimalDistance))
                ) {
                    continue;
                }
...
function loadProblemSets(jsonDir) {
    ...
    if (!fs.existsSync(setsFile)) {
        ...
        return {
            ...
            medium: problems
                .filter((p) => p.cities.length > 80 && p.cities.length <= 150)
                .map((p) => p.name),
            large: problems
                .filter((p) => p.cities.length > 150 && p.cities.length <= 200)
                .map((p) => p.name),
    ...
    try {
        ...
        const newError = new Error(`Error loading problem sets: ${error.message}`);
        newError.cause = error;
        throw newError;
    }
}
...
function getProblemIndex(jsonDir) {
    ...
    try {
        ...
        const newError = new Error(`Error loading problem index: ${error.message}`);
        newError.cause = error;
        throw newError;
    }
}
