...
class KPathOptimizer {
    ...
    *search(node, currentPath, currentG, kBudget, visitedSet) {
        ...
        const neighbors = this.getNeighbors(node).filter((n) => !visitedSet.has(`${n.x},${n.y}`));
    ...
    try {
        ...
        if (msg.type === 'init_and_run') {
            ...
            if (config.algorithm === 'k') {
                optimizer = new KPathOptimizer(
                    config.width,
                    config.height,
                    config.startNode,
                    config.endNode,
                    walls,
                    config.options.pessimistic,
                    config.options.maxK
                );
            } else {
                optimizer = new AStarOptimizer(
                    config.width,
                    config.height,
                    config.startNode,
                    config.endNode,
