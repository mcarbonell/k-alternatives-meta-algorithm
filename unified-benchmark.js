...
class BenchmarkRunner {
    ...
    async runTSPBenchmark() {
        ...
        for (const problem of CONFIG.tsp.problems) {
            ...
            for (const k of CONFIG.tsp.kValues) {
                ...
                console.log(
                    ` Success: ${stats.successRate}% | Gap: ${stats.meanGap.toFixed(2)}% | Time: ${stats.avgTime.toFixed(1)}s`
                );
    ...
    async runKnapsackBenchmark() {
        ...
        for (const problem of CONFIG.knapsack.problems) {
            ...
            for (const k of CONFIG.knapsack.kValues) {
                ...
                console.log(
                    ` Best: ${stats.best} | Gap: ${stats.meanGap.toFixed(2)}% | Time: ${stats.avgTime.toFixed(1)}s`
                );
    ...
    generateReport() {
        ...
        console.log('=================');
        console.log(
            `TSP: ${this.results.summary.tsp.avgSuccessRate.toFixed(1)}% success rate, ${this.results.summary.tsp.avgGap.toFixed(2)}% avg gap`
        );
        console.log(
            `Knapsack: Best gap ${this.results.summary.knapsack.bestGap.toFixed(2)}%, avg gap ${this.results.summary.knapsack.avgGap.toFixed(2)}%`
        );
    ...
    summarizeTSP() {
        ...
        return {
            ...
            avgSuccessRate:
                tspResults.reduce((sum, r) => sum + r.successRate, 0) / tspResults.length,
    ...
    summarizeKnapsack() {
        ...
        return {
            ...
            avgGap:
                knapResults.reduce((sum, r) => sum + Math.abs(r.meanGap), 0) / knapResults.length,
