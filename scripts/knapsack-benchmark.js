...
async function main() {
    ...
    for (const k of [0, 1, 2]) {
        ...
        const bar = k === 0 ? '⬜' : res.value > trapBase ? '🟩' : '➖';
    ...
    for (const type of TYPES) {
        ...
        for (const k of [0, 1, 2, 3]) {
            ...
            const improvement =
                k === 0 ? 0 : (((res.value - baselineVal) / baselineVal) * 100).toFixed(2);
            const gap = (((maxRelaxedVal - res.value) / maxRelaxedVal) * 100).toFixed(2);

            let barLabel;
            if (k === 0) barLabel = '⬜ (Greedy)';
            else if (parseFloat(improvement) > 0) barLabel = '🟩 (Improved)';
            else barLabel = '➖';

            console.log(
                `  K=${k} | Val: ${res.value} | Gap to Bound: ${gap}% | Improv: +${improvement}% | Time: ${res.time}ms | ${barLabel}`
            );
        }
        console.log('-'.repeat(50));
    }
